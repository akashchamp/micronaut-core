...
class Lowering:
    ...
    def _sibling_call(self, name, node):
        ...
        if hint is None:
            if function_def.hasReturnValue():
                self._refuse("unknown-type", f"the method [{name}] returns a value but has no return hint", node)
            return_type = VOID
        elif hint.name() == "None":
            return_type = VOID
        else:
        ...
        if declares:
            ...
            arguments = [self._coerce(argument, parameter_type, argument_node)
                         for argument, parameter_type, argument_node in zip(lowered_arguments, parameter_types, node.args)]
            self.java_calls += 1
            call = InvokeSibling(name, parameter_types, arguments, return_type, "java")
        else:
            self.bridge_calls += 1
    ...
    def _bool_op(self, node):
        ...
        if all(value.type() == BOOLEAN for value in values):
            ...
            return result
        if len(values) == 2 and values[0].type() == values[1].type() and isinstance(node.values[0], (ast.Name, ast.Constant)):
            # `x or default` on values of one type: the left operand is pure, so it may be tested and yielded
            first, second = values
            test = self._truthy(node.values[0])
            return Conditional(test, first, second, first.type()) if isinstance(node.op, ast.Or) else Conditional(test, second, first, first.type())
    ...
    def _joined(self, node):
        ...
        for value in node.values:
            if isinstance(value, ast.Constant):
                ...
                parts.append(self._expression(value.value))
            else:
